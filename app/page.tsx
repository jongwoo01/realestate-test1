...
const serviceProcessSteps = [
  ...
  { step: "02", title: "요구 확인", desc: "거래 목적, 일정, 가격 기준, 공개 범위를 먼저 확인합니다." },
  { step: "03", title: "맞춤 전략 수립", desc: "최신 시세와 매물 상태를 분석해 최적의 방향을 제안합니다." },
  { step: "04", title: "안전한 계약", desc: "계약 조건 확인과 사후 관리까지 단계별로 진행합니다." },
];

const qualitySignals = [
  { title: "요구 확인", desc: "매도·전세·월세 목적과 희망 일정을 먼저 분리해 상담 방향을 잡습니다." },
  { title: "입력 검증", desc: "필수 정보와 연락처 형식을 확인해 잘못된 상담 접수를 줄입니다." },
  { title: "지역 일관성", desc: "강동구, 길동, 성내동, 둔촌동 중심 사례로 서비스 범위를 명확히 유지합니다." },
  { title: "오류 회복", desc: "접수 실패 시 사용자가 다시 시도할 수 있도록 구체적인 메시지를 제공합니다." },
];

const reviewItems = [
  { title: "거래 목적", desc: "빠른 매도, 공실 해소, 전세 만기 대응처럼 상담 목표를 먼저 확인합니다." },
  { title: "가격 기준", desc: "희망가와 주변 실거래 흐름을 비교해 조정 가능 범위를 정리합니다." },
  { title: "일정 제약", desc: "이사일, 잔금일, 기존 임차인 만기일을 기준으로 진행 순서를 잡습니다." },
  { title: "공개 범위", desc: "온라인 공개, 제한 공개, 대기 고객 매칭 중 적절한 노출 방식을 선택합니다." },
];

const riskControls = [
  "상담 전 개인정보는 최소한으로만 받습니다.",
  "전화 연결 전 접수 목적과 지역을 먼저 확인합니다.",
  "계약 가능성이 낮은 조건은 초기에 조정안을 제시합니다.",
  "광고보다 실제 문의 품질과 일정 리스크를 우선합니다.",
];

const casePreviews = [
  { type: "아파트 매도", title: "강동구 길동 월드파크 2단지", result: "3주 내 계약 완료", desc: "이사 일정 때문에 빠른 매도 상담. 가격 조정과 적극적인 온라인 홍보로 단기간 성사." },
  { type: "오피스텔 월세", title: "강동구 성내동 성내스테이", result: "2주 내 계약 완료", desc: "긴 공실 기간으로 고민. 사진 재정리와 조건 조정 후 문의 응대로 빠른 임차인 확보." },
  { type: "빌라 전세", title: "강동구 둔촌동 주택가", result: "보증금 조정 성공", desc: "기존 세입자 만기 일정에 맞춘 전세 상담. 주변 시세 비교 및 합리적 안내로 계약." },
];
  ...
  return (
    <div className="flex flex-col w-full">
          ...
          <div className="w-full md:w-1/3 flex justify-center">
            <div className="relative w-64 h-80 rounded-2xl overflow-hidden bg-white shadow-xl flex flex-col">
              <div className="flex-1 bg-gradient-to-b from-gray-200 to-gray-300 flex items-center justify-center">
                <svg width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1" className="text-gray-400" aria-hidden="true">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 pt-6">
              <div className="bg-white p-6 rounded-xl shadow-sm border border-muted/50">
                <h3 className="font-bold text-lg text-primary mb-2 flex items-center gap-2">
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-accent" aria-hidden="true"><polyline points="20 6 9 17 4 12"></polyline></svg>
                  빠른 응답과 소통
                </h3>
      ...
      <section className="py-24 px-6 bg-white">
        <div id="process" className="max-w-5xl mx-auto">
      ...
      <section className="py-20 px-6 bg-muted/40">
        <div id="quality" className="max-w-5xl mx-auto">
          <div className="text-center mb-12">
            <h2 className="text-3xl md:text-4xl font-bold text-primary mb-4">상담 품질 기준</h2>
            <p className="text-lg text-foreground/80">빠른 접수보다 정확한 상담 연결을 우선합니다.</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-5">
            {qualitySignals.map((item) => (
              <article key={item.title} className="bg-white rounded-2xl border border-muted p-6 shadow-sm">
      ...
      <section className="py-24 px-6 bg-white">
        <div className="max-w-5xl mx-auto">
          <div className="mb-12 max-w-3xl">
            <h2 className="text-3xl md:text-4xl font-bold text-primary mb-4">상담 전에 확인하는 것</h2>
            <p className="text-lg text-foreground/80 leading-relaxed">
              같은 매도 상담이라도 가격, 일정, 공개 범위가 다르면 진행 방식이 달라집니다.
              처음부터 확인 항목을 나누어 불필요한 재상담을 줄입니다.
            </p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-5">
            {reviewItems.map((item) => (
              <article key={item.title} className="rounded-2xl border border-muted bg-muted/25 p-6">
                <h3 className="font-bold text-primary mb-3">{item.title}</h3>
                <p className="text-sm leading-relaxed text-foreground/75">{item.desc}</p>
              </article>
            ))}
          </div>
          <div className="mt-8 rounded-2xl bg-[#1A2B4C] p-6 text-white md:p-8">
            <h3 className="text-xl font-bold mb-5">상담 리스크를 줄이는 운영 기준</h3>
            <ul className="grid gap-3 text-sm leading-6 text-white/80 md:grid-cols-2">
              {riskControls.map((item) => (
                <li key={item} className="flex gap-3">
                  <span className="mt-2 h-2 w-2 shrink-0 rounded-full bg-[#C5A880]" aria-hidden="true"></span>
                  <span>{item}</span>
                </li>
              ))}
            </ul>
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {casePreviews.map((item) => (
              <div key={item.title} className="bg-white/5 rounded-2xl p-8 backdrop-blur-sm border border-white/10 hover:bg-white/10 transition-colors flex flex-col h-full">
      ...
      <section className="py-24 px-6 bg-white">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="space-y-4">
            {faqs.map((faq) => (
              <details key={faq.q} className="group bg-muted/40 rounded-xl overflow-hidden [&_summary::-webkit-details-marker]:hidden border border-muted">
                <summary className="flex items-center justify-between cursor-pointer p-6 font-bold text-primary text-lg">
                  ...
                  <span className="transition-transform group-open:rotate-180 flex-shrink-0 text-accent">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><polyline points="6 9 12 15 18 9"></polyline></svg>
                  </span>
                </summary>
